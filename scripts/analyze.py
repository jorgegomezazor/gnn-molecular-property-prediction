"""Aggregate per-run summaries, build the comparison table and figures.
...
def load_summary(results_dir: str, model: str, seed: int):
    run_name = f"{model}_seed{seed}"
    path = os.path.join(results_dir, run_name, "summary.json")
    if not os.path.exists(path):
        print(f"[Analyze] No summary for {run_name}, skipping.")
        return None
    with open(path) as f:
        return json.load(f)
...
def extract_embeddings(model, loader, device: str):
    """Run a forward pass capturing the input to the regression head."""
    model.eval()
    all_emb, all_tgt = [], []
    buffer = {}

    def hook_fn(module, inp, out):
        buffer["emb"] = inp[0].detach().cpu()
